<script lang="ts">
	...
	import { lastRoundPoints } from '$lib/stats';
...
<section class="space-y-6">
	<div>
		<h1 class="text-2xl font-bold">מחזור {data.current.number}</h1>
		<p class="text-sm text-slate-400">
			רמזור יריבות (לוגו בטבעת צבעונית), מחיר, נקודות וזמינות — להרכב + מעקב.
		</p>
	</div>

	<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
		<h2 class="mb-3 font-semibold text-emerald-300">משחקים קרובים</h2>
		{#if data.hasFixtures}
			<ul class="space-y-3">
		...
		{:else}
			<p class="text-sm text-slate-500">
				אין משחקים במסד עדיין. הריצו <code class="text-emerald-300">bun run db:import-fixtures</code>
	...
	<div class="space-y-2">
		<h2 class="font-semibold text-sky-300">הקבוצה + מעקב</h2>
		{#each data.focusPlayers as r}
			<PlayerRow
				...
				points={lastRoundPoints(r.player)}
